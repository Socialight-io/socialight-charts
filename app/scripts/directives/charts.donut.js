    ...
    return {
      ...
      link: function (scope, element, attrs) {
	    ...
	    scope.create = function () { 
	    	...
	    	if (scope.donutchart.length <= 0) { return false; }
	        ...
	        colors.domain(d3.extent(scope.donutchart, function(v) {
	     	...
	     	if (!scope.donutchart || scope.donutchart.length == 0 || !element.width()) { return false; }
		 	...
		 	if (scope.options.height) { element.height(scope.options.height); } 
		 	else { element.height(scope.radius*2); }
			...
			if (scope.options.label) { 
		        ...
		        scope.vis.append("svg:text")
		            ...
		            .text(si(d3.sum(scope.data, function (d) { return d[scope.options.stack.key]; })));
	        ...
	        if (scope.data.length) {
		        ...
		        scope.arcs.append("svg:path")
		            ...
		            .attr("fill", function(d, i) { return scope.options.stack.colors(d) || colors(d.value); } )
		            .style("opacity", .7)
